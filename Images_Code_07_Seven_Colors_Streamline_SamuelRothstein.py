...
#Check to see if user wants to use a preset
use_preset = raw_input("Would you like to use a preset? (Answer 'Yes' or 'No'): ")
#If they do, get a filename and read the file data
if use_preset == 'Yes':
    filename_valid = False
    while filename_valid == False:
    ...
    color_components = list(range(21))
    gs_breaks = list(range(6))
    ...
    color6_R = color_components[18]
    color6_G = color_components[19]
    color6_B = color_components[20]
    gs_break0 = gs_breaks[0]
    gs_break1 = gs_breaks[1]
    gs_break2 = gs_breaks[2]
    gs_break3 = gs_breaks[3]
    gs_break4 = gs_breaks[4]
    gs_break5 = gs_breaks[5]
    
#If the user does not want to use a preset, set all color components and grayscale breaks to 0
else:
    color0_R = 0
    color0_G = 0
    color0_B = 0
...
color6_paper = numpy.zeros((img_height, img_width, img_channels), numpy.uint8)
...
cv2.createTrackbar('GS5_position','Grayscale Sliders', gs_break5, 255, lambda x:None)
...
#Color 6
cv2.createTrackbar('color6_R','Color 6 Components', color6_R, 255, lambda x:None)
cv2.createTrackbar('color6_G','Color 6 Components', color6_G, 255, lambda x:None)
cv2.createTrackbar('color6_B','Color 6 Components', color6_B, 255, lambda x:None)
...
while (keypressed != 27 and keypressed != ord('s')):
    ...
    gs_break5 = cv2.getTrackbarPos('GS5_position','Grayscale Sliders')
    ...
    #Color 5
    color6_R = cv2.getTrackbarPos('color6_R','Color 6 Components')
    color6_G = cv2.getTrackbarPos('color6_G','Color 6 Components')
    color6_B = cv2.getTrackbarPos('color6_B','Color 6 Components')
    ...
    color6_paper[0:img_height, 0:img_width, 0:img_channels] = [color6_B,color6_G,color6_R]
    ...
    color5_max = [gs_break5,gs_break5,gs_break5]
    
    color6_min = [gs_break5+1,gs_break5+1,gs_break5+1]
    color6_max = [255,255,255]
    ...
    color6_min = numpy.array(color6_min, dtype = "uint8")
    color6_max = numpy.array(color6_max, dtype = "uint8")
    ...
    color6_mask = cv2.inRange(gs_img, color6_min, color6_max)
    ...
    color6_part = cv2.bitwise_or(color6_paper, color6_paper, mask = color6_mask)
    
    #Add each colors part to create one custom image
    custom_img = cv2.bitwise_or(cv2.bitwise_or(cv2.bitwise_or(cv2.bitwise_or(cv2.bitwise_or(cv2.bitwise_or(color0_part, color1_part), color2_part), color3_part), color4_part), color5_part), color6_part)
    
    #Show custom image in a seperate window
...
#If s is pressed, save grayscale and custom images
    ...
    color_components = [color0_R,color0_G,color0_B, color1_R,color1_G,color1_B, color2_R,color2_G,color2_B, color3_R,color3_G,color3_B, color4_R,color4_G,color4_B, color5_R,color5_G,color5_B, color6_R,color6_G,color6_B]
    gs_breaks = [gs_break0,gs_break1,gs_break2,gs_break3,gs_break4,gs_break5]
